    ...
    <form action="{{ route('admin.bai-viet.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="bf-card">
                ...
                <div class="bf-form-row">
                    <div class="bf-form-group" style="grid-column: 1/-1">
                        ...
                        <input type="text" name="tieuDe" value="{{ old('tieuDe') }}" placeholder="Nhập tiêu đề bài viết..."
                            class="{{ $errors->has('tieuDe') ? 'is-invalid' : '' }}">
                        @error('tieuDe')
                ...
                <div class="bf-form-row">
                    <div class="bf-form-group" style="grid-column: 1/-1">
                        ...
                        <textarea name="tomTat" rows="3"
                            placeholder="Mô tả ngắn gọn nội dung bài viết (hiển thị ở danh sách)...">{{ old('tomTat') }}</textarea>
                        <span class="form-hint">Tối đa 500 ký tự. Nếu để trống, hệ thống sẽ lấy đoạn đầu của nội
                            dung.</span>
                    </div>
                </div>
            </div>
        ...
        <div class="bf-tab-panel" id="tab-noidung">
            <div class="bf-card">
                ...
                <div class="bf-editor-wrap">
                    <textarea name="noiDung" id="noiDung-editor">{!! old('noiDung') !!}</textarea>
                </div>
                @error('noiDung')
        ...
        <div class="bf-tab-panel" id="tab-phanloai">
                ...
                @else
                    <div class="bf-category-grid">
                        @foreach ($danhMucs as $dm)
                            <label>
                                <input type="checkbox" name="danhMucIds[]" value="{{ $dm->danhMucId }}" {{ in_array($dm->danhMucId, old('danhMucIds', [])) ? 'checked' : '' }}>
            ...
            <div class="bf-card">
                ...
                <input type="hidden" name="tagNames" id="tagNamesHidden" value="{{ old('tagNames') }}">
            ...
            <div class="bf-card">
                ...
                <div class="bf-toggle-group">
                    <label class="bf-toggle">
                        <input type="hidden" name="trangThai" value="{{ old('trangThai', 1) }}">
                    ...
                    <span class="bf-toggle-label" id="toggle-label">
                        {{ old('trangThai', 1) ? 'Xuất bản ngay' : 'Lưu bản nháp' }}
                    </span>
                </div>
                <p class="form-hint" style="margin-top:10px">
                    Bài viết đã xuất bản sẽ hiển thị trên trang blog cho khách hàng xem.
                </p>
            </div>
        </div>
...
@section('script')
    ...
    <script>
        ...
        @if ($errors->has('tieuDe') || $errors->has('anhDaiDien') || $errors->has('tomTat'))
            document.querySelector('[data-tab="tab-thongtin"]').click();
        @elseif ($errors->has('noiDung'))
            document.querySelector('[data-tab="tab-noidung"]').click();
        @endif
    </script>
@endsection

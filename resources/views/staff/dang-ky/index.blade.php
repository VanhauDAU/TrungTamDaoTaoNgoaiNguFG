...
@section('content')
    ...
    <div class="container-fluid px-0">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
            ...
            <a href="{{ route($portalRouteBase . '.dang-ky.create') }}" class="btn btn-primary">
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body">
                <form method="GET" action="{{ route($portalRouteBase . '.dang-ky.index') }}" class="row g-3 align-items-end">
                    <div class="col-lg-4">
                ...
                <table class="table align-middle mb-0">
                    ...
                    <tbody>
                        @forelse ($registrations as $registration)
                            @php
                                $currentStatus = (int) $registration->trangThai;
                                ...
                                $canConfirm = in_array($currentStatus, [
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    <div class="d-flex flex-wrap justify-content-end gap-2">
                                        <form action="{{ route($portalRouteBase . '.dang-ky.confirm', $registration->dangKyLopHocId) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="btn btn-sm btn-outline-primary" {{ !$canConfirm ? 'disabled' : '' }}>
                                                Xác nhận
                                            </button>
                                        </form>
                                        <form action="{{ route($portalRouteBase . '.dang-ky.hold', $registration->dangKyLopHocId) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="btn btn-sm btn-outline-secondary" {{ !$canHold ? 'disabled' : '' }}>
                                                Bảo lưu
                                            </button>
                                        </form>
                                        <form action="{{ route($portalRouteBase . '.dang-ky.restore', $registration->dangKyLopHocId) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="btn btn-sm btn-outline-success" {{ !$canRestore ? 'disabled' : '' }}>
                                                Khôi phục
                                            </button>
                                        </form>
                                        <button type="button" class="btn btn-sm btn-outline-dark"
                                            data-bs-toggle="modal" data-bs-target="#transferModal{{ $registration->dangKyLopHocId }}" {{ !$canTransfer ? 'disabled' : '' }}>
                                            Chuyển lớp
                                        </button>
                                        <form action="{{ route($portalRouteBase . '.dang-ky.cancel', $registration->dangKyLopHocId) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="btn btn-sm btn-outline-danger" {{ !$canTransfer ? 'disabled' : '' }}>
                                                Hủy
                                            </button>
                                        </form>
                                    </div>
        ...
        <div class="modal fade" id="transferModal{{ $registration->dangKyLopHocId }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="{{ route($portalRouteBase . '.dang-ky.transfer', $registration->dangKyLopHocId) }}" method="POST">
                        @csrf
                        @method('PATCH')

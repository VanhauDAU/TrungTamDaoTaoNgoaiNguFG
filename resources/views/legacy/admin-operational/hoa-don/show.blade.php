    ...
    <div class="invoice-detail-shell">
        <section
            ...
            <div class="invoice-detail-hero__topbar">
                <a href="{{ route($portalRouteBase . '.hoa-don.index') }}" class="invoice-back-link">
                    <i class="fas fa-arrow-left"></i>
                    <span>Quay lại danh sách</span>
                </a>
                <div class="invoice-detail-hero__action-btns">
                    <a href="{{ route($portalRouteBase . '.hoa-don.print', $hoaDon->hoaDonId) }}" target="_blank"
                        class="btn btn-light btn-sm border">
                        <i class="fas fa-print me-1"></i> In hóa đơn
                    </a>
                    @if ($hoaDon->taiKhoan?->email)
                        <button type="button" class="btn btn-light btn-sm border"
                            onclick="requestDocumentEmail('{{ route($portalRouteBase . '.hoa-don.email', $hoaDon->hoaDonId) }}', '{{ $hoaDon->taiKhoan->email }}', 'hóa đơn {{ $maHD }}')">
                            <i class="fas fa-envelope me-1"></i> Gửi email
                        </button>
                    @else
                        <button type="button" class="btn btn-light btn-sm border"
                            onclick="requestDocumentEmail('{{ route($portalRouteBase . '.hoa-don.email', $hoaDon->hoaDonId) }}', '', 'hóa đơn {{ $maHD }}')">
                            <i class="fas fa-envelope me-1"></i> Gửi email
                        </button>
                    @endif
                    @if ($coTheThuTien)
                            ...
                            @foreach ($hoaDon->phieuThus->sortByDesc('ngayThu')->values() as $phieuThu)
                                ...
                                <article class="receipt-item {{ $hopLe ? '' : 'is-cancelled' }}">
                                    ...
                                    <div class="d-flex gap-2 align-items-start">
                                        <a href="{{ route($portalRouteBase . '.hoa-don.phieu-thu.print', $phieuThu->phieuThuId) }}"
                                            target="_blank" class="btn btn-outline-secondary btn-sm">
                                            <i class="fas fa-print"></i> In
                                        </a>
                                        <button type="button" class="btn btn-outline-primary btn-sm"
                                            onclick="requestDocumentEmail('{{ route($portalRouteBase . '.hoa-don.phieu-thu.email', $phieuThu->phieuThuId) }}', '{{ $hoaDon->taiKhoan?->email ?? '' }}', 'phiếu thu {{ $maPhieuThu }}')">
                ...
                <div class="invoice-sidebar-card">
                    ...
                    <div class="invoice-side-panel is-active" data-panel="summary">
                        ...
                        <div class="invoice-side-panel" data-panel="edit">
                            <form action="{{ route($portalRouteBase . '.hoa-don.update', $hoaDon->hoaDonId) }}" method="POST"
                        ...
                        <div class="invoice-side-panel {{ $errors->any() ? 'is-active' : '' }}" data-panel="receipt">
                            @if ($conNo > 0)
                                <form action="{{ route($portalRouteBase . '.hoa-don.phieu-thu.store', $hoaDon->hoaDonId) }}" method="POST"
...
@section('script')
    <script>
        ...
        @if (session('autoPrintReceiptId'))
            window.open(@json(route($portalRouteBase . '.hoa-don.phieu-thu.print', session('autoPrintReceiptId'))), '_blank');
        @endif

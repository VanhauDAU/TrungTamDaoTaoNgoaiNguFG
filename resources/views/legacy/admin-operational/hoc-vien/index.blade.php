...
@section('content')
    @php
        ...
        $portalPathBase = $portalRouteBase === 'staff' ? '/staff' : '/admin';
    @endphp
    ...
    <div class="hv-page-header">
        ...
        <div style="display:flex;gap:10px;align-items:center">
            ...
            <a href="{{ route($portalRouteBase . '.hoc-vien.trash') }}" class="btn-add-student"
            ...
            <a href="{{ route($portalRouteBase . '.hoc-vien.create') }}" class="btn-add-student">
    ...
    {{-- ── Filter bar ─────────────────────────────────────────────── --}}
    <form action="{{ route($portalRouteBase . '.hoc-vien.index') }}" method="GET" class="hv-filter-bar" id="filter-form">
        {{-- Search --}}
        ...
        <a href="{{ route($portalRouteBase . '.hoc-vien.index') }}" class="btn-filter btn-filter-reset">
            <i class="fas fa-times"></i> Đặt lại
        </a>
        <a href="{{ route($portalRouteBase . '.hoc-vien.export', request()->query()) }}" class="btn-filter btn-filter-primary"
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="hv-actions">
                                        <a href="{{ route($portalRouteBase . '.hoc-vien.edit', $hv->taiKhoan) }}"
        ...
        function confirmDelete(username, name) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.action = `{{ url($portalPathBase . '/hoc-vien') }}/${encodeURIComponent(username)}`;
                    form.submit();
                }
            });
        }

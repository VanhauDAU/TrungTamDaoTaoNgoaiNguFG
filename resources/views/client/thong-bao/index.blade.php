...
@section('content')
    {{-- Push thêm spacing vì header fixed --}}
    <div style="padding-top:90px;"></div>

    <div class="nb-client-wrapper">

        {{-- ── PAGE HEADER ───────────────────────────────────────── --}}
        <div class="nb-client-header">
            <h1 class="nb-client-title">
                <div class="title-icon"><i class="fas fa-bell"></i></div>
                Thông báo của tôi
                @if ($tongChuaDoc > 0)
        ...
        <div class="nb-filter-tabs">
            ...
            <a href="{{ route('home.thong-bao.index', ['filter' => 'unread']) }}"
                ...
                Chưa đọc
                @if ($tongChuaDoc > 0)
                    <span class="nb-tab-count">{{ $tongChuaDoc > 99 ? '99+' : $tongChuaDoc }}</span>
                @endif
            </a>
        ...
        @if ($items->isNotEmpty())
            <div class="nb-list" id="nb-list">
                @foreach ($items as $item)
                    ...
                    <div class="nb-item {{ $isUnread ? 'unread' : '' }}" data-id="{{ $tb->thongBaoId }}"
                        ...
                        <div class="nb-item-body">
                            <div class="nb-item-title">
                                @if ($tb->ghim)
                                    <i class="fas fa-thumbtack me-1" style="color:#f59e0b;font-size:.75rem;"></i>
                                @endif
                                {{ $tb->tieuDe }}
                            </div>
                            <div class="nb-item-preview">{{ Str::limit(strip_tags($tb->noiDung), 120) }}</div>
                    ...
                    {{-- Modal nội dung đầy đủ (click sẽ mở) --}}
                @endforeach
            </div>
...
@section('script')
    <script>
        @php
            // Chuẩn bị data cho modal chi tiết (tách riêng để Blade không bị lỗi parse)
            $nbItemsData = [];
            foreach ($items as $i) {
                $tb = $i->thongBao;
                if (!$tb) {
        ...
        // Click mở modal chi tiết và mark read
        document.querySelectorAll('.nb-item[data-id]').forEach(item => {
            ...
            <span class="badge bg-secondary-subtle text-secondary">${nb.uuTien}</span>`;
                ...
                if (this.dataset.read === '0') {
                    ...
                    if (dot) dot.remove();
                    // Gọi mark read API (NB_MARK_READ_URL từ partials/script)

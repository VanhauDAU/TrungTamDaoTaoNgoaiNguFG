...
class BaiVietService implements BaiVietServiceInterface
{
    public function getList(Request $request): array
    {
        ...
        if ($search = $request->q) $query->search($search);
        if ($request->filled('danhMucId')) $query->whereHas('danhMucs', fn($q) => $q->where('danhmucbaiviet.danhMucId', $request->danhMucId));
        if ($request->filled('tagId'))     $query->whereHas('tags', fn($q) => $q->where('tags.tagId', $request->tagId));
    ...
    public function store(Request $request): BaiViet
    {
        ...
        if ($request->filled('danhMucIds')) $baiViet->danhMucs()->sync($request->danhMucIds);
    ...
    public function update(Request $request, int $id): BaiViet
    {
        ...
        if ($request->tieuDe !== $baiViet->tieuDe) $data['slug'] = $this->generateUniqueSlug($request->tieuDe, $id);
    ...
    {
        if (empty(trim($tagNames))) { $baiViet->tags()->detach(); return; }
    ...
    private function generateUniqueSlug(string $name, ?int $excludeId = null): string
    {
        ...
        $candidate = $slug; $counter = 1;
        while (true) {
            $q = BaiViet::where('slug', $candidate);
            if ($excludeId) $q->where('baiVietId', '!=', $excludeId);
            if (!$q->exists()) break;

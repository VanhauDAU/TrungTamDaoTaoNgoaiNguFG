...
namespace App\Services\Internal\LienHe;
...
class LienHeService implements LienHeServiceInterface
{
    public function getList(Request $request): array
    {
        ...
        if ($request->filled('nguoiPhuTrachId')) {
            if ($request->nguoiPhuTrachId === '__null__') {
                $query->whereNull('nguoiPhuTrachId');
            } else {
        ...
        if (in_array(strtolower($orderBy), $allowed, true) && in_array($dir, ['asc', 'desc'], true)) {
            $query->orderBy($orderBy, $dir);

            if ($orderBy !== 'lienHeId') {
                $query->orderBy('lienHeId', 'desc');
            }
        } else {
            $query->orderByDesc('created_at')->orderByDesc('lienHeId');
        }
    ...
    {
        return [
            'lienHe' => LienHe::with(['lichSu', 'phanHoi', 'nguoiPhuTrach.hoSoNguoiDung'])->findOrFail($id),
            'nhanVienList' => TaiKhoan::with('hoSoNguoiDung')
                ->select('taikhoan.taiKhoanId', 'taikhoan.taiKhoan')
                ->where('role', TaiKhoan::ROLE_NHAN_VIEN)
                ->orderBy('taikhoan.taiKhoan')
    ...
    public function assign(Request $request, string $id): array
    {
        ...
        $lienHe = LienHe::with('nguoiPhuTrach.hoSoNguoiDung')->findOrFail($id);
        $oldPhuTrach = $lienHe->nguoiPhuTrach?->hoSoNguoiDung?->hoTen ?? 'Chưa gán';
        $newUser = $request->nguoiPhuTrachId
            ? TaiKhoan::with('hoSoNguoiDung')
                ->where('role', TaiKhoan::ROLE_NHAN_VIEN)
                ->findOrFail($request->nguoiPhuTrachId)
            : null;
    ...
    public function storeReply(Request $request, string $id): array
    {
        ...
        $phanHoi = LienHePhanHoi::create([
            ...
            'tenNguoiGui' => $user?->hoSoNguoiDung?->hoTen ?? $user?->taiKhoan ?? 'Nhân sự',
            'daGuiEmail' => false,
        ]);

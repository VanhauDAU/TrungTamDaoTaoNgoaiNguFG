...
class CourseController extends Controller
{
    ...
    private function validateClassRegistration($user, $class): bool|string
    {
        ...
        // 3. Kiểm tra học viên đã đăng ký lớp này chưa
        ...
        // Chỉ load lớp trangThai 1 (đang mở) hoặc 4 (đang học)
        $activeRegistrations = DangKyLopHoc::where('taiKhoanId', $user->taiKhoanId)
            ...
            ->get()
            ->filter(fn($reg) => $reg->lopHoc !== null);

        // TẦNG 1: kiểm tra theo từng buổi học cụ thể
